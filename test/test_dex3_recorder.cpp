// Records ONLY the Dex3 hands (rt/dex3/<side>/state, ~830 Hz each) —
// one-stream isolation runner (the kist_data_collector runner records
// everything enabled). Runs them even when `dex3.enabled` is false in the
// config; everything else stays off.
//
